"""
LLM-as-a-Judge Uncertainty — BMP Project  |  Condition A: BERT-only baseline
BERT embeddings → TubeNet (Tube Loss, Anand et al. 2024) + Conformal Prediction
No LLM score used as input feature.
"""
...
OUT_DIR       = "bmp_results_A"
os.makedirs(OUT_DIR, exist_ok=True)

MAX_SAMPLES   = 200
ALPHA         = 0.10
T_CONF        = 1.0 - ALPHA       # 0.9
R_SHIFT       = 0.0
HIDDEN        = 256
EPOCHS        = 150
LR            = 3e-4
...
print("=" * 62)
print("  Condition A — BERT-only baseline (no LLM score)")
print(f"  TubeNet + Tube Loss  |  t={T_CONF}  DELTA={DELTA}")
print(f"  device={DEVICE}  α={ALPHA}  per_dim={PER_DIMENSION}")
print("=" * 62)
...
# ══════════════════════════════════════════════════════════
#  LLM JUDGE  (only used to fetch ground-truth LLM score
#              for the DEMO printout — NOT passed to model)
# ══════════════════════════════════════════════════════════
client = Groq(api_key=os.environ.get("GROQ_API_KEY"))

def llm_judge_score(article: str, summary: str) -> float:
    """Single LLM call — result used only in demo printout, not as a feature."""
    key = summary[:150]
...
def tube_loss(mu1: torch.Tensor, mu2: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    ...
    final_loss = torch.where(inside, loss_part1, loss_part2) \
                 + DELTA * torch.abs(f1 - f2)
    return final_loss.mean()

# ══════════════════════════════════════════════════════════
#  LOAD DATA  — Condition A: X = BERT embedding only
# ══════════════════════════════════════════════════════════
...
for sample in tqdm(raw, desc="Features"):
    ...
    z  = get_bert_embedding(art, summ)   # 768-dim — the ONLY feature
    yh = llm_judge_score(art, summ)      # fetched for demo only, NOT appended to X

    X_list.append(z)                     # ← no LLM score here
    Y_dims_list.append(dim_scores)
    llm_scores_list.append(yh)           # kept separately for demo comparison
    time.sleep(0.03)
...
N      = len(Y_med)
print(f"[Data] N={N}  X={X.shape}  (BERT-only, no LLM score feature)")
...
IN_DIM = X.shape[1]   # 768 (pure BERT, no LLM feature)
...
def train_and_eval(y_tr, y_cal, y_te, label="combined"):
    ...
    # MAE of raw LLM score vs human (for comparison — LLM not used as feature)
    mae_l = float(np.mean(np.abs(llm_scores[idx_te] - y_te)))

    print(f"  [{label:12s}]  cov={cov_c:.3f}  wid={wid_c:.3f}  "
          f"MAE_mid={mae_m:.3f}  MAE_llm(ref)={mae_l:.3f}  q̂={q_hat:.3f}")
...
results = {}
if PER_DIMENSION:
    print(f"\n[Train] Per-dimension  (Condition A — BERT only)")
...
# ══════════════════════════════════════════════════════════
print("\n" + "═"*70)
print(f"  {'Dimension':<14} {'Coverage':>9} {'Width':>8} {'MAE_mid':>9} {'MAE_llm(ref)':>13} {'q̂':>7}")
print("─"*70)
for k, r in results.items():
    print(f"  {k:<14} {r['cov_c']:>9.3f} {r['wid_c']:>8.3f} "
          f"{r['mae_mid']:>9.3f} {r['mae_llm']:>13.3f} {r['q_hat']:>7.3f}")
print("═"*70)
print("  Note: MAE_llm(ref) = raw LLM score vs human (not a model feature here)")
...
dyh    = llm_judge_score(da, dsu)   # for display only

dz  = get_bert_embedding(da, dsu)
dx  = torch.tensor(dz.reshape(1, -1), dtype=torch.float32).to(DEVICE)  # BERT only
m_demo.eval()
with torch.no_grad():
    dm1, dm2 = m_demo(dx)
...
print("\n" + "═"*55)
print("  DEMO — Condition A (BERT-only)")
print("═"*55)
print(f"  Human ground truth          : {d_true:.2f}")
print(f"  LLM raw score (ref only)    : {dyh:.0f}  ← NOT used as input feature")
...
def mae_bar(ax):
    ...
    ax.bar(x - w/2, mae_l, w, label="Raw LLM (ref)",      color="#E57373", alpha=0.9)
    ax.bar(x + w/2, mae_m, w, label="Midpoint (BERT-only)", color="#4DB6AC", alpha=0.9)
    ax.set_xticks(x); ax.set_xticklabels(keys, fontsize=8)
    ax.set_ylabel("MAE vs Human")
    ax.set_title("MAE: Raw LLM vs BERT-only Midpoint", fontsize=10, fontweight="bold")
...
def midpoint_vs_truth(ax, r, title):
    ...
    ax.set_xlabel("Human Ground Truth", fontsize=9)
    ax.set_ylabel("Midpoint", fontsize=9)
    ax.set_title(title, fontsize=10, fontweight="bold")
...
fig1.suptitle(
    f"Condition A — BERT-only  |  TubeNet + Tube Loss (t={T_CONF}) + Conformal Prediction",
...
if PER_DIMENSION:
    ...
    fig2.suptitle(f"Condition A — Per-Dimension Intervals (α={ALPHA})",
...
fig3.suptitle("Condition A — Diagnostics", fontsize=12, fontweight="bold")
...
if PER_DIMENSION:
    ...
    fig4.suptitle("Condition A — Tube Loss Training Curves per Dimension",
                  fontsize=12, fontweight="bold")
...
#  SAVE JSON
# ══════════════════════════════════════════════════════════
save_json(os.path.join(OUT_DIR, "results_A.json"), {
    k: {"coverage":     round(r["cov_c"],  4),
        "avg_width":    round(r["wid_c"],  4),
        "midpoint_mae": round(r["mae_mid"],4),
        "raw_llm_mae":  round(r["mae_llm"],4),
        "q_hat":        round(r["q_hat"],  4),
        "condition":    "A_bert_only",
        "alpha": ALPHA, "t": T_CONF, "delta": DELTA}
    for k, r in results.items()})
print(f"[Saved] {OUT_DIR}/results_A.json\nDone.")